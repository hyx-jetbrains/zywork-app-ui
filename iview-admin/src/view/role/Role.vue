  ...
  <div>
    ...
    >
      <Form ref="addForm" :model="form" :label-width="80" :rules="validateRules">
        ...
        <FormItem label="角色描述" prop="description">
          <Input v-model="form.description" type="textarea" :autosize="descriptionAutoSize" placeholder="请输入角色描述"/>
        </FormItem>
        <FormItem label="是否默认" prop="isDefault">
          <Select v-model="form.isDefault" placeholder="请选择是否默认角色" filterable>
            <Option v-for="item in isDefaultSelect" :value="item.value" :key="item.value">{{item.label}}</option>
          </Select>
        </FormItem>
      </Form>
      <div slot="footer">
    ...
    <Modal v-model="modal.search" title="高级搜索">
      <Form ref="searchForm" :model="searchForm" :label-width="80">
        ...
        <FormItem label="是否默认" prop="isDefault">
          <Select v-model="searchForm.isDefault" placeholder="请选择是否默认角色" clearable filterable>
            <Option v-for="item in isDefaultSelect" :value="item.value" :key="item.value">{{item.label}}</option>
          </Select>
        </FormItem>
        <FormItem label="版本号">
    ...
    >
      ...
      <p>是否默认角色:
        <span v-text="form.isDefault === 1 ? '是':'否' "></span>
      </p>
      <p>版本号:
...
export default {
  ...
  methods: {
    ...
    userOpt(itemName, row) {
      ...
      } else if (itemName === 'perminssion') {
        this.$refs.rolePermission.initData(row.id)
      }
    },
    add() {
      utils.add(this)
