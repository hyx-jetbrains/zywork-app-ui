  ...
  <div>
    ...
    >
      <Form ref="addForm" :model="form" :label-width="80" :rules="validateRules">
        ...
        <FormItem label="地址" prop="tempAddress">
  ...
  methods: {
    ...
    userOpt(itemName, row) {
      if (itemName === 'showEdit') {
        this.tempAddress = []
        this.tempAddress.push(row.province)
        this.tempAddress.push(row.city)
        this.tempAddress.push(row.district)
    ...
    add() {
      if(this.tempAddress.length <= 0) {
        ...
        this.form.province = this.tempAddress[0]
      } else if (this.tempAddress.length === 2) {
        this.form.province = this.tempAddress[0]
        this.form.city = this.tempAddress[1]
      } else if (this.tempAddress.length === 3) {
        this.form.province = this.tempAddress[0]
        this.form.city = this.tempAddress[1]
        this.form.district = this.tempAddress[2]
      }
      this.form.city = this.tempAddress[1]
      this.form.district = this.tempAddress[2]
      utils.add(this)
